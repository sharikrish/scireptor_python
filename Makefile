...
# raw data INFILES
#todb_reads_files = $(addprefix $(dir)/, $(notdir$(wildcard $(raw_data)/*.fasta))) 
todb_reads_files = $(addprefix $(dir)/, $(notdir $(patsubst %.fasta,%.todb_reads_done,$(wildcard $(raw_data)/*.fasta)))) $(addprefix $(dir)/, $(notdir $(patsubst %.fastq,%.todb_reads_done,$(wildcard $(raw_data)/*.fastq))))
...
define do_illumina_data
	./todb_reads.py -fq $< -ri $<.info -m $(experiment_id)
	touch $@
endef
...
$(dir)/consensusfasta.done: $(dir)/todb_consensus_tags.done
	./fromdb_consensus_fasta.py -p $(dir)
...
%.rrzout: %.rfasta
	...
	mv $< $$file_temp_fasta; \
	./perform_RazerS.py -f $$file_temp_fasta -ro $@.razers; \
	mv $$file_temp_fasta $<; \
	mv $@.razers $@
...
$(dir)/$(notdir %.todb_reads_done): $(raw_data)/%.fasta
	@echo $<
	$(call do_454_data)
	
$(dir)/$(notdir %.todb_reads_done): $(raw_data)/%.fastq
	@echo $<
	$(call do_illumina_data)
